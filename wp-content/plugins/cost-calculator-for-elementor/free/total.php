...
class Total_Format_Cost_Calculator_Widget extends \Elementor\Widget_Base{
	...
	protected function render() {
		...
		$number_format_symbols = esc_html($settings["number_format_symbols"]);
		$number_format_decimal_sep = esc_html($settings["number_format_decimal_sep"]);
		$number_format_thousand_sep = esc_html($settings["number_format_thousand_sep"]);
		$number_format_num_decimals = esc_html($settings["number_format_num_decimals"]);
		if( $settings["number_format"] == "yes" ){
			$class .="elementor-number-format";
			$atts .='data-a-sign="'.$number_format_symbols.'" ';
			$atts .='data-a-dec="'.$number_format_decimal_sep.'" ';
			$atts .='data-a-sep="'.$number_format_thousand_sep.'" ';
			$atts .='data-m-dec="'.$number_format_num_decimals.'" ';
			if( $settings['number_format_symbols_position'] == "right" ){
				$atts .='data-p-sign="s" ';
			}
		}
				?>
		<input readonly="readonly" data-formula="<?php echo wp_kses_post($settings["formula"])  ?>" <?php echo wp_kses_post($atts) ?> class="<?php echo esc_attr($class) ?>" type="text" name="<?php echo esc_attr($name) ?>" id="<?php echo esc_attr($name) ?>">
		<?php
	}
}